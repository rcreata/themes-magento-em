...
class EM_Blog_Adminhtml_TagController extends Mage_Adminhtml_Controller_Action
{
		...
		if(!$data['custom_design_from'])
			...
			try {
			        ...
			        //them hoac chinh sua du lieu cho blog_url_rewrite  
                            $dataBlogUrl = array('post_id' => '','tag_id'=>$id,'cat_id'=>'','request_path'=>'tag/'.$data['tag_identifier'].'.html');
                            Mage::helper('blog')->saveAndUpdateUrl($dataBlogUrl,$id,'tag');
    ...
    {
        $identifier = $this->getRequest()->getPost('tag_identifier');
        if(!$identifier)
             $identifier = Mage::helper('blog/post')->friendlyURL($this->getRequest()->getPost('name'));
        else
             $identifier = Mage::helper('blog/post')->friendlyURL($identifier);


        $response = new Varien_Object();
        $response->setError(false);
        $blogUrl = Mage::getModel('blog/blogurl');
        $flag = $blogUrl->validate('tag/'.$identifier,$this->getRequest()->getParam('id'),'tag');
        if($flag)
        {
            $response->setError(true);
            $response->setAttribute("Tag Identifier");
            $response->setMessage("Tag identifier is unique");
            $response->setData('identifier',$identifier);
        }
