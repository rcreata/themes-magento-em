...
class EM_Blog_Block_Adminhtml_Category_Tree extends EM_Blog_Block_Adminhtml_Category_Abstract
{
    ...
    protected function _getNodeJson($node, $level = 0)
    {
        ...
        if (is_array($node)) {
            $node = new Varien_Data_Tree_Node($node, 'id', new Varien_Data_Tree);
        }
        ...
        $rootForStores = in_array($node->getId(), array(1));
        $item['id']  = $node->getId();
...
//if($node->getId()==5){print_r($node->getChildren());exit;}
    ...
    {
        $result = $this->htmlEscape($node->getCatName());
