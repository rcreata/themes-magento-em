 ...
 */
class EM_Blog_Model_Mysql4_Tree extends Varien_Data_Tree_Dbp
{
    ...
    public function __construct()
    {
        ...
        $con = $resource->getConnection('catalog_write');
        $con->setFetchMode(2);
        parent::__construct(
            $con,
            $resource->getTableName('blog_category'),
            array(
                Varien_Data_Tree_Dbp::ID_FIELD       => 'id',
                Varien_Data_Tree_Dbp::PATH_FIELD     => 'path',
                Varien_Data_Tree_Dbp::ORDER_FIELD    => 'cat_name',
    ...
    public function addCollectionData($collection=null, $sorted=false, $exclude=array(), $toLoad=true, $onlyActive = false)
    {
        ...
        $collection->addFieldToFilter('id',array('in'=>$nodeIds));
        if ($onlyActive) {

            $disabledIds = $this->_getDisabledIds($collection);
            if ($disabledIds) {
                $collection->addFieldToFilter('id', array('nin'=>$disabledIds));
            }
    ...
    protected function _getDefaultCollection($sorted=false)
    {
        ...
        if ($sorted) {
            if (is_string($sorted)) {
                ...
                $collection->addAttributeToSort('cat_name');
            }
        }
